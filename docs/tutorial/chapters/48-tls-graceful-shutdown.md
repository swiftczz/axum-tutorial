...
把第 46 章 graceful shutdown 和第 47 章 TLS rustls 合在一起。启动 HTTPS + HTTP 重定向两个服务,理解 `axum_server::Handle` 和 `with_graceful_shutdown` 分别如何关闭两个服务,同一个终止信号同时影响两者。
...
和第 46 章区别:信号到来后不只返回,而是主动调 `handle.graceful_shutdown(Some(Duration::from_secs(10)))`——给 TLS server 最多 10 秒完成优雅关闭(源码注释提到这和 Docker 强制关闭等待时间有关)。
...
HTTP redirect server 用普通 `axum::serve` 所以继续用第 46 章的 `with_graceful_shutdown(signal)`。`signal` 是泛型 future(`Future<Output = ()> + Send + 'static`),就是前面创建的 `shutdown_signal(handle.clone())`——**HTTP 服务和 HTTPS 服务共享同一个关闭触发源**。
...
**为什么 HTTP redirect server 还能用 `with_graceful_shutdown`?** HTTP redirect server 用普通 `axum::serve`,所以继续用第 46 章方式。
