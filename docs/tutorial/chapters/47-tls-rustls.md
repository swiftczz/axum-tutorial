...
**handler 转 service**:`redirect.into_make_service()` 把 handler closure 变成可交给 `axum::serve` 的 service(见第 29 章 handler/service 互转)。这个 HTTP 服务不需 Router,只做"所有请求重定向到 HTTPS"一件事。
