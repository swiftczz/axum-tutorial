...
**为什么用 `futures_channel::mpsc` 不用 tokio channel?** 测试需要实现 `Sink` 和 `Stream` 的 channel,`futures_channel::mpsc` 正好满足。

## 常见问题

**WebSocket 测试一定要启动真实服务器吗?** 不一定。要覆盖 upgrade/路由/真实协议用集成测试;只测消息处理逻辑用泛型 Sink/Stream 单元测试。

**为什么单元测试不用 tokio channel?** 需要实现 `Sink`+`Stream` 的 channel,`futures_channel::mpsc` 适合。
...
- 单元测试的关键技巧:**把业务逻辑从具体 `WebSocket` 类型解耦**,只依赖 `Sink<Message>` + `Stream<Item = Result<Message, _>>`,测试时用 `futures_channel::mpsc` 模拟读写。
