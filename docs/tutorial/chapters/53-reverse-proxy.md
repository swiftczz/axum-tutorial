...
```text
正向代理:客户端知道代理存在,代理帮客户端访问外部网站(第 52 章)
...
反向代理需主动请求后端,所以需 HTTP client。client 可复用(同第 16 章 reqwest Client),放进 state 后每个 handler 用同一个。
...
反向代理还必须处理 **hop-by-hop headers**(见第 52 章):`Connection`/`Transfer-Encoding`/`Keep-Alive` 等只在相邻两跳有效,不能原样透传。`hyper-util` client 在本示例自动处理了一部分,手写别的转发逻辑要按 RFC 7230 §6.1 剥离。
