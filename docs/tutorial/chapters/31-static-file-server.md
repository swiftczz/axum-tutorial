...
**2. SPA fallback**(3002):两层 fallback:
...
- 响应状态码默认 200:内容返回 index.html 让前端路由接管。不再显式设 404——SPA 常见做法是由前端路由判断真实页面是否存在。
...
- SPA fallback 用 `ServeDir::new(...).not_found_service(ServeFile::new("assets/index.html"))`:找不到文件时返回 index.html 让前端路由接管(响应默认 200,不再显式设 404)。
