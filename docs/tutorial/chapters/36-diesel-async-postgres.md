...
模式和第 34 章 tokio-postgres 几乎一样:从 state 取 pool → `pool.get_owned()` 借 owned connection → 包装。`FromRequestParts` 合适因为不读 body。
...
`up.sql`、`table!`、`User`、`NewUser` 都和第 35 章一样。**本章不再内嵌 migration**,改用 diesel CLI 在启动前手动 `diesel migration run`(见"运行"小节),保持 example 代码精简。
