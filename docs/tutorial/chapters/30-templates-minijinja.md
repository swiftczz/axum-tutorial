...
handler 返回 `Result<Html<String>, StatusCode>`,但内部用多个 `unwrap()`(模板注册在启动时,example 为突出主流程)。真实项目应把模板找不到/渲染失败转成 500 响应并记日志,也可像第 29 章封装统一模板响应类型。
