...
不用 `axum-server::bind_rustls`(第 47 章高层封装),手写 TCP accept loop,用 `tokio-rustls` 完成 TLS 握手后交给 Hyper/Axum。这是"低层 TLS 三部曲"(53 rustls / 54 native-tls / 55 openssl)的基础,讲透完整 accept loop + TLS + Hyper 桥接链路。
...
大多数项目用第 47 章 `axum_server::bind_rustls` 就够。需要手动写低层 TLS 的场景:
...
**ALPN 必须设**——`auto::Builder` 根据 ALPN 协商结果选 HTTP/2 还是 HTTP/1.1。不设的话浏览器请求 HTTP/2 会失败(见第 47 章 ALPN 解释)。
