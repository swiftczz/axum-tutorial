...
> 本示例 handler 里 `socket` 在 `tokio::select!` 中被移动,依赖 WebSocket 操作的 **cancel-safety**(第 45 章测试会讲——`WebSocket::recv` 和 `broadcast::Receiver::recv` 都 cancel-safe,可安全用在 select!)。
